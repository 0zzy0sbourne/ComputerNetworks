...
class DistanceVectorRouting(Thread):
    ...
    def make_pkt(self, packet):
        ...
        router = struct.pack('=1s', packet.Router.encode('iso-8859-1'))
...
if __name__ == "__main__":
    ...
    for node in network_topology['nodes']:
        # Create a Router instance for the end node. 
        router = Router (
        ...
        # Store the router in the dictionary
        routers[node["node_id"]] = router
    # Now we have a dictionary of routers that we can use in our program
    print(routers[node["node_id"]])
    print(routers[node["node_id"]].routerName)
